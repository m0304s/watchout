...
# 단일 소스 전처리 스트림 (/multiview에서 사용)
@app.get("/cctv")
async def cctv_stream(request: Request, src: str = Query(...), mirror: bool = True):
    cap, status = try_open_capture(src, timeout_sec=5)
    if cap is None:
        raise HTTPException(status_code=503, detail=f"소스를 열 수 없음: {status}")
    cap.release()
    ...
    async def gen():
        it = mjpeg_generator(int(src) if src.isdigit() else src, mirror=mirror)
        try:
            while True:
                if await request.is_disconnected():
                    break
