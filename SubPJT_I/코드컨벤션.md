...
### 1.3 JWT 토큰에서 UUID 추출

#### JWT 토큰 처리
- **`@AuthenticationPrincipal Object principal`** 사용
- **UUID 추출**: `principal.toString()`

```java
// Controller에서 JWT 토큰에서 UUID 추출
@PostMapping("/posts")
...
// Service에서 UUID 사용
public PostResponseDto createPost(PostRequestDto dto, String userUuid) {
